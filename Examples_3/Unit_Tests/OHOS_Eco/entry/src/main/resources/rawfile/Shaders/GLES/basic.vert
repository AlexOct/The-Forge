...
// 2024-05-29 11:14:55.061572
// "..\..\src\01_Transformations\Shaders\FSL\basic.vert.fsl"
//--------------------------------------
...
STRUCT(VSInput)
{
#define _Position_662
	DATA(float3, Position, POSITION);
#define _Normal_666
	DATA(float3, Normal,   NORMAL);
};
#ifdef _Position_662
RES_IN(float3, Position);
#endif
#ifdef _Normal_666
RES_IN(float3, Normal);
#endif

STRUCT(VSOutput)
{
#define _Position_694
	DATA(float4, Position, SV_Position);
#define _Color_698
	DATA(float4, Color,    COLOR);
};
#ifdef _Color_698
RES_OUT(float4, Color);
#endif

uniform int SV_INSTANCEID;
...
void main()
{
	...
	In.Normal = Normal;
#endif
	uint InstanceID = uint(SV_INSTANCEID);
    VSOutput Out;

    float4x4 tempMat = mul(Get_MVP, Get_TOWORLD(InstanceID));
    Out.Position = mul(tempMat, float4(In.Position.xyz, 1.0));

    float4 normal = normalize(mul(Get_TOWORLD(InstanceID), float4(In.Normal.xyz, 0.0))); // Assume uniform scaling
    float4 pos = mul(Get_TOWORLD(InstanceID), float4(In.Position.xyz, 1.0));
    ...
    if (Get_COLOR(InstanceID).w < 0.01) // Special case for Sun, so that it is lit from its top
        lightDir = float3(0.0, 1.0, 0.0);
    else
        lightDir = normalize(Get_LIGHTPOSITION - pos.xyz);

    // float distance = length(lightDir);
    // float attenuation = 1.0 / (quadraticCoeff * distance * distance);
    // float intensity = lightIntensity * attenuation;

    float3 baseColor = Get_COLOR(InstanceID).xyz;
    float3 blendedColor = (Get_LIGHTCOLOR * baseColor) * lightIntensity;
