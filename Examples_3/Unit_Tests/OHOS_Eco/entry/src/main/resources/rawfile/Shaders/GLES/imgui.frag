...
// 2024-05-28 14:59:11.866757
// "..\..\..\..\..\Common_3\OS\UI\Shaders\FSL\imgui.frag.fsl"
//--------------------------------------
...
#define Get_PROJECTIONMATRIX float4x4(uniformBlockVS[0], uniformBlockVS[1], uniformBlockVS[2], uniformBlockVS[3])
uniform float4 uniformBlockVS[4];
...
void main()
{
	PS_INPUT In;
#ifdef _pos_284
	In.pos = float4(float4(gl_FragCoord.xyz, 1.0 / gl_FragCoord.w));
#endif
#ifdef _col_288
	In.col = col;
#endif
...
#endif
	float4 Out;
	Out = In.col * SampleTex2D(Get_UTEX, Get_USAMPLER, In.uv);
	{
		gl_FragColor = Out;
	}
}
