...
void main()
{
	...
	if(side == 1)
    {
        ...
        Out = SampleTex2D(Get_RIGHTTEXT, Get_USAMPLER0, newtextcoord);
    }
    else if (side == 2)
    {
        newtextcoord = (In.TexCoord.zy) / 20.0 + 0.5;
        newtextcoord = float2(newtextcoord.x, 1.0 - newtextcoord.y);
        Out = SampleTex2D(Get_LEFTTEXT, Get_USAMPLER0, newtextcoord);
    }
    else if (side == 4)
    {
        newtextcoord = (In.TexCoord.xz) / 20.0 + 0.5;
        newtextcoord = float2(newtextcoord.x, 1.0 - newtextcoord.y);
        Out = SampleTex2D(Get_BOTTEXT, Get_USAMPLER0, newtextcoord);
    }
    else if (side == 5)
    {
        newtextcoord = (In.TexCoord.xy) / 20.0 + 0.5;
        newtextcoord = float2(newtextcoord.x, 1.0 - newtextcoord.y);
        Out = SampleTex2D(Get_FRONTTEXT, Get_USAMPLER0, newtextcoord);
    }
    else if (side == 6)
    {
        newtextcoord = (In.TexCoord.xy) / 20.0 + 0.5;
        newtextcoord = float2(1.0 - newtextcoord.x, 1.0 - newtextcoord.y);
        Out = SampleTex2D(Get_BACKTEXT, Get_USAMPLER0, newtextcoord);
    }
	else
    {
        newtextcoord = (In.TexCoord.xz) / 20.0 + 0.5;
        newtextcoord = float2(newtextcoord.x, newtextcoord.y);
        Out = SampleTex2D(Get_TOPTEXT, Get_USAMPLER0, newtextcoord);
    }
    {
    	gl_FragColor = Out;
    }
