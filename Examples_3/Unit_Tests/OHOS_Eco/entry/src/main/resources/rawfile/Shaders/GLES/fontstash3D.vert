...
// 2024-05-28 14:59:10.921454
// "..\..\..\..\..\Common_3\OS\Fonts\Shaders\FSL\fontstash3D.vert.fsl"
//--------------------------------------
...
#define Get_MVP float4x4(uniformBlock_rootcbv[0], uniformBlock_rootcbv[1], uniformBlock_rootcbv[2], uniformBlock_rootcbv[3])
uniform float4 uniformBlock_rootcbv[4];

#define Get_UTEX0 uTex0
RES(Tex2D(float4), uTex0, UPDATE_FREQ_NONE, t2, binding = 2);
#define Get_USAMPLER0 NO_SAMPLER

#define Get_COLOR float4(uRootConstants[0])
#define Get_SCALEBIAS float2(uRootConstants[1].x, uRootConstants[1].y)
uniform float4 uRootConstants[2];

#endif

STRUCT(VsIn)
{
#define _position_422
	DATA(float2, position, Position);
#define _texCoord_426
	DATA(float2, texCoord, TEXCOORD0);
};
#ifdef _position_422
RES_IN(float2, Position);
#endif
#ifdef _texCoord_426
RES_IN(float2, UV);
#endif
...
STRUCT(PsIn)
{
#define _position_454
	DATA(float4, position, SV_Position);
#define _texCoord_458
	DATA(float2, texCoord, TEXCOORD0);
};
#ifdef _texCoord_458
...
#endif
	PsIn Out;
	Out.position = mul(Get_MVP, float4(In.position * Get_SCALEBIAS.xy, 1.0, 1.0));
